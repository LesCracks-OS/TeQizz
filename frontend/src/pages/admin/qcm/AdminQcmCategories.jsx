...
const empty = { name: "", slug: "", description: "", displayOrder: 0, isActive: true };

export default function AdminQcmCategories() {
  ...
  const openEdit = (cat) => {
    setForm({ name: cat.name, slug: cat.slug, description: cat.description ?? "", displayOrder: cat.displayOrder ?? 0, isActive: cat.isActive });
    setError("");
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === "create") {
        await adminService.createQcmCategory(form);
      } else {
        await adminService.updateQcmCategory(modal.id, form);
      }
      setModal(null);
      load();
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">Slug</th>
            ...
            {loading ? (
              ...
              categories.map(cat => (
                <tr key={cat.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{cat.questionsCount ?? 0}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${cat.isActive ? "bg-emerald-500/10 text-emerald-500" : "bg-muted text-muted-foreground"}`}>
                      {cat.isActive ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => openEdit(cat)} className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button onClick={() => setConfirmDelete(cat)} className="p-1.5 rounded hover:bg-red-500/10 transition-colors text-muted-foreground hover:text-red-500">
      ...
      {/* Create/Edit Modal */}
      {modal && (
        <Modal title={modal.mode === "create" ? "New Category" : "Edit Category"} onClose={() => setModal(null)}>
          <div className="space-y-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button disabled={saving} onClick={handleSave} className="px-4 py-2 text-sm rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50">
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
          </div>
      ...
      {confirmDelete && (
        <Modal title="Delete Category?" onClose={() => setConfirmDelete(null)}>
          <p className="text-sm text-muted-foreground mb-4">
            Delete <strong>{confirmDelete.name}</strong>? Categories with questions cannot be deleted.
          </p>
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={() => handleDelete(confirmDelete.id)} className="px-4 py-2 text-sm rounded-lg bg-red-500 text-white hover:bg-red-600 transition-colors">Delete</button>
          </div>
        </Modal>
      )}
    </div>
